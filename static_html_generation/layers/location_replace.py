...
class LocationReplace(object):
    ...
    def update_offsets(self, original, text):
        ...
        self.offset_counters = range(self.offset_starter, self.offset_starter + num_offsets)
    ...
    def apply_layer_to_text(self, original, replacement, text):
        self.update_offsets(original, text)
        offset = self.offsets[self.counter]
    ...
    def location_replace(self, xml_node, original, replacement, locations):
        if node.text:
            ...
            while self.counter < len(locations) and locations[self.counter] in self.offsets:
                xml_node.text = self.apply_layer_to_text(original, replacement, xml_node.text)
        ...
        if node.tail:
            ...
            while self.counter < len(locations) and locations[self.counter] in self.offsets:
                xml_node.tail = self.apply_layer_to_text(original, replacement, xml_node.tail)
            self.update_offset_starter()
